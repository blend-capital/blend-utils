...
async function mock(addressBook: AddressBook) {
  ...
  await comet.bundleBind(
    [addressBook.getContractId('BLND'), addressBook.getContractId('USDC')],
    [BigInt(1_000e7), BigInt(25e7)],
  ...
  await backstop.update_token_val(config.admin);
  ...
  await oracle.setAssetPrices(
    [
      ...
      { price: BigInt(0.05e7), assetId: addressBook.getContractId('BLND') },
    ],
    config.admin
  );
