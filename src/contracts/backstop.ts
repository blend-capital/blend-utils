...
export async function installBackstop(stellarRpc: Server, config: Config, source: Keypair) {
  const operation = createInstallOperation('backstop', config);
  await invokeStellarOperation(stellarRpc, config, operation, source);
}

export async function deployBackstop(stellarRpc: Server, config: Config, source: Keypair) {
