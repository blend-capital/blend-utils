...
export async function installEmitter(stellarRpc: Server, source: Keypair, config: Config) {
  const operation = createInstallOperation('emitter', config);
  await invokeStellarOperation(stellarRpc, config, operation, source);
}

export async function deployEmitter(stellarRpc: Server, source: Keypair, config: Config) {
