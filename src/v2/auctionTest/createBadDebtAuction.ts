...
if (process.argv.length < 4) {
  throw new Error(
    'Arguments required (in decimal): `network` `account name` `pool name` `bid assets`'
  );
}
const keypair = config.getUser(process.argv[3]);
const pool = addressBook.getContractId(process.argv[4]);
const bid: string[] = JSON.parse(process.argv[5]);
...
await createBadDebtAuction(txParams, pool, bid);
