...
async function submit(addressBook: AddressBook) {
  const whale = config.getUser('FRODO');
  console.log('FRODO: ', whale.publicKey());
  await airdropAccount(whale);
  const usdc_token = new TokenClient(addressBook.getContractId('BLND'));
  const usdc_asset = new Asset('BLND', config.admin.publicKey());
  ...
  const stellarPool = new PoolClient(addressBook.getContractId('BlndBOOST'));

  const stellarRequests: Request[] = [
    {
      amount: BigInt(1e7),
      request_type: RequestType.SupplyCollateral,
      address: addressBook.getContractId('BLND'),
    },
    // {
    //   amount: BigInt(10),
